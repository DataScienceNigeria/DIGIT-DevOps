terraform {
  backend "s3" {
    bucket = "digit-lts-githubactions-bucket-dsn"
    ...
    dynamodb_table = "digit-lts-githubactions-bucket-dsn"
...
module "db" {
  ...
  engine_version                = "16"   ## postgres version
  storage_type                  = "gp2"
...
module "eks" {
  ...
  node_groups = {
    spot = {
      ...
      max_capacity        = var.number_of_worker_nodes
      min_capacity        = 1
